...
class IncomeAccount extends DataObject {
	...
	function getCMSFields($includeDOMS = true) {
		...
		$mainTab->push(new TextField('Name', _t('IncomeAccount.NAME', 'Name')));
		$mainTab->push(new FieldGroup(
				new NumericField('Account', _t('IncomeAccount.ACCOUNT', 'Account')),
				new NumericField('ExpensePlace', _t('IncomeAccount.EXPENSEPLACE', 'Expenseplace'))
		));
	...
	static function toDropdownList() {
		$objects = DataObject::get('IncomeAccount');
		$list = array('' => '');
