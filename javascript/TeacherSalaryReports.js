jQuery(function() {	
	jQuery("#GenerateReportButton").click(function() {
		...
		*/
		...
		var dialogTitle = ss.i18n._t('TeacherSalaryReports.TITLE', 'Salary report');
		...
		var lockAllText = ss.i18n._t('TeacherSalaryReports.LOCKALL', 'Lock all payment months');
		var unlockAllText = ss.i18n._t('TeacherSalaryReports.UNLOCKALL', 'Unlock all payment months');		
		...
		buttonsOptions[pdfText] = function() {
			...
			var queryData = jQuery("form#Form_TeacherSalaryReportsForm").formSerialize();
			queryData += '&PDF=true';
		...
		buttonsOptions[lockAllText] = function() {
			jQuery(".ui-button:button").attr("disabled","disabled").addClass('ui-state-disabled');
			...
			jQuery.ajax({
				url: 'admin/coursebooking/lockPaymentMonth',
				data: queryData,
				...
				error: function() {
					jQuery(".ui-button:button").attr("disabled",false).removeClass('ui-state-disabled');
					alert('Error locking payment months');
				}
			});
		};
		buttonsOptions[unlockAllText] = function() {
			jQuery(".ui-button:button").attr("disabled","disabled").addClass('ui-state-disabled');
			...
			jQuery.ajax({
				...
				error: function() {
					jQuery(".ui-button:button").attr("disabled",false).removeClass('ui-state-disabled');
					alert('Error unlocking payment months');
				}
			});			
		};		
		...
		jQuery('#ReportDialog').dialog({
			...
			open: function() {
				jQuery(".ui-dialog-buttonset").css('width', '100%');
				jQuery(".ui-button:button").last().css('float', 'left');
				jQuery(".ui-button:button").last().prev().css('float', 'left');
